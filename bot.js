...
//const client = new Client({ puppeteer: { executablePath: 'C:/Program Files/Google/Chrome/Application/chrome.exe', headless: false }, session: sessionCfg });
...
client.on('message', async msg => {
    ...
    if (msg.body === '!ping reply') {
            ...
            if (groups.length == 0) {
              ...
              groups.forEach((group, i) => {
                const participants = group.participants;
                    ...
                    },1000 + Math.floor(Math.random() * 2000) * (i+1)  )
                    });
                //group.sendMessage(newMsgGroup);
        ...
        let newMentions = msg.body.slice(7);
        let text = "";
        let mentions = [];
        ...
        client.getChats().then(chats => {
            ...
            if (groups.length == 0) {
              ...
              groups.forEach((group, i) => {
                const start2 = async function(user) {
                for(let participant of group.participants) {
                    //const contact = await client.getContasctById(participant.id._serialized);
                    //mentions.push(contact);
                    text += `@${participant.id.user} `;
                    //client.sendMessage(`${participant.id.user}@c.us`,newMentions);
                }
                group.sendMessage(text + newMentions, { mentions });
                //chat.sendMessage(newMentions);
            }
            start2();
              });
            }
